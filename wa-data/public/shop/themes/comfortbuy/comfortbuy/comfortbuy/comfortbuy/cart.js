$(function () {
    ...
    $(".cart a.delete").click(function () {
        ...
        $.post('delete/', {html: sumbolrub, id: tr.data('id')}, function (response) {
            if (response.data.count == 0) {
                //location.reload();
                location.href = $('.cart').find('a.button.back').attr('href');
            }
            tr.remove();
    ...
    $(".cart input.qty").change(function () {
        ...
        if (that.val() > 0) {
            ...
            if (that.val()) {
                $.post('save/', {html: sumbolrub, id: tr.data('id'), quantity: that.val()}, function (response) {
    ...
    $(".cart .services input:checkbox").change(function () {
        ...
        if ($(this).is(':checked')) {
           ...
           var data = {html: sumbolrub, parent_id: parent_id, service_id: $(this).val()};
           ...
           $.post('delete/', {html: sumbolrub, id: div.data('id')}, function (response) {
               div.data('id', null);
               tr.find('.item-total').html(response.data.item_total);
               updateCart(response.data);
           }, "json");
    ...
    $(".cart .services select").change(function () {
        var tr = $(this).closest('tr');
        $.post('save/', {html: sumbolrub, id: $(this).closest('div').data('id'), 'service_variant_id': $(this).val()}, function (response) {
            tr.find('.item-total').html(response.data.item_total);
            updateCart(response.data);
        }, "json");
    });
