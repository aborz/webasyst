<?php
/**
 * User ideas in customer account, and submit controller for it.
 */
class shopFrontendMyIdeasAction extends waMyProfileAction
{
    public function execute()
    {
        parent::execute();

        $this->view->assign('my_nav_selected', 'ideas');

        // Set up layout and template from theme
        $this->setThemeTemplate('my.ideas.html');
        if (!waRequest::isXMLHttpRequest()) {
            $this->setLayout(new shopFrontendLayout());
            $this->getResponse()->setTitle(_w('Ideas'));
