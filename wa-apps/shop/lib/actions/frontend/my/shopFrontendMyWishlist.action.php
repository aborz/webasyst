<?php
/**
 * User wishlist in customer account, and submit controller for it.
 */
class shopFrontendMyWishlistAction extends waMyProfileAction
{
    public function execute()
    {
        parent::execute();

        $this->view->assign('my_nav_selected', 'wishlist');

        // Set up layout and template from theme
        $this->setThemeTemplate('my.wishlist.html');
        if (!waRequest::isXMLHttpRequest()) {
            $this->setLayout(new shopFrontendLayout());
            $this->getResponse()->setTitle(_w('wishlist'));
