<?php
/**
 * User feedback in customer account, and submit controller for it.
 */
class shopFrontendMyFeedbackAction extends waMyProfileAction
{
    public function execute()
    {
        parent::execute();

        $this->view->assign('my_nav_selected', 'feedback');

        // Set up layout and template from theme
        $this->setThemeTemplate('my.feedback.html');
        if (!waRequest::isXMLHttpRequest()) {
            $this->setLayout(new shopFrontendLayout());
            $this->getResponse()->setTitle(_w('Feedback'));
