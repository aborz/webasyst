<?php
/**
 * User contests in customer account, and submit controller for it.
 */
class shopFrontendMyContestsAction extends waMyProfileAction
{
    public function execute()
    {
        parent::execute();

        $this->view->assign('my_nav_selected', 'contests');

        // Set up layout and template from theme
        $this->setThemeTemplate('my.contests.html');
        if (!waRequest::isXMLHttpRequest()) {
            $this->setLayout(new shopFrontendLayout());
            $this->getResponse()->setTitle(_w('Contests'));
