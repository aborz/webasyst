<?php
/**
 * User selaspecial in customer account, and submit controller for it.
 */
class shopFrontendMySelaSpecialAction extends waMyProfileAction
{
    public function execute()
    {
        parent::execute();

        $this->view->assign('my_nav_selected', 'selaspecial');

        // Set up layout and template from theme
        $this->setThemeTemplate('my.selaspecial.html');
        if (!waRequest::isXMLHttpRequest()) {
            $this->setLayout(new shopFrontendLayout());
            $this->getResponse()->setTitle(_w('SelaSpecial'));
