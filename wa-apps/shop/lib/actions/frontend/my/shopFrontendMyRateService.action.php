<?php
/**
 * User rateservice in customer account, and submit controller for it.
 */
class shopFrontendMyRateServiceAction extends waMyProfileAction
{
    public function execute()
    {
        parent::execute();

        $this->view->assign('my_nav_selected', 'rateservice');

        // Set up layout and template from theme
        $this->setThemeTemplate('my.rateservice.html');
        if (!waRequest::isXMLHttpRequest()) {
            $this->setLayout(new shopFrontendLayout());
            $this->getResponse()->setTitle(_w('RateService'));
